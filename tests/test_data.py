...
from src.data import UbuntuDataSet, UbuntuDataLoader, collate
...
class TestDataSet(unittest.TestCase):
    ...
    def test_dataset(self):
        ...
        for (ctx, response, cands) in dataloader:
            ...
            self.assertEqual(response.shape, torch.Size([self.batch_size, self.seq_len]))
            self.assertEqual(cands.shape, torch.Size([self.batch_size, self.cands_len, self.seq_len]))
            break
